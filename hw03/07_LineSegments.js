...
let intersectionPoints = []; // 교점 좌표들 (flat array)
...
function setupMouseEvents() {
    ...
    function CirclePointMake(){
        ...
        for (let i = 0; i <= segments; i++) {
            ...
            const x = radius * Math.cos(angle) + center_x;
            const y = radius * Math.sin(angle) + center_y;
    ...
    function handleMouseUp() {
        if (isDrawing && tempEndPoint) {
            ...
            if (lines.length == 1) {
                updateText(textOverlay, "Circle: center (" + center_x.toFixed(2) + ", " + center_y.toFixed(2) + 
                ...
                const intersections = getCircleLineIntersections(center_x, center_y, radius, x1, y1, x2, y2);


                switch(intersections.length){
                    case 0: updateText(textOverlay3, "No intersection"); break;
                    case 1: updateText(textOverlay3, "Intersection Points: 1 Point 1: (" + intersections[0][0].toFixed(2) + ", " + intersections[0][1].toFixed(2) + ")"); break;
                    case 2: updateText(textOverlay3, "Intersection Points: 2 Point 1: (" + intersections[0][0].toFixed(2) + ", " + intersections[0][1].toFixed(2) + ") Point 2: (" + intersections[1][0].toFixed(2) + ", " + intersections[1][1].toFixed(2) + ")"); break;
                }

                intersectionPoints = []; // 초기화
                for (let pt of intersections) {
                    intersectionPoints.push(pt[0], pt[1]);
                }
            }
    ...
    function getCircleLineIntersections(cx, cy, r, x1, y1, x2, y2) {
        const dx = x2 - x1;
        const dy = y2 - y1;
...
function render() {
    ...
    for (let line of lines) {
        if (num == 0) { // 첫 번째 선분인 경우, yellow
            shader.setVec4("u_color", [0.8, 0.0, 1.0, 1.0]);
            gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(circlePoint), gl.STATIC_DRAW);
            gl.bindVertexArray(vao);
            gl.drawArrays(gl.LINE_LOOP, 0, circlePoint.length / 2);
        }
        else { // num == 1 (2번째 선분인 경우), red
            shader.setVec4("u_color", [0.75, 0.75, 1.0, 1.0]);
    ...
    //draw intersectino Points
    if (intersectionPoints.length > 0) {
        shader.setVec4("u_color", [1.0, 1.0, 0.0, 1.0]); // 노란색 점
    ...
    console.log("교점", intersectionPoints);
